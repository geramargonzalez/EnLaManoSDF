define(['N/log', 'N/record', 'N/search', 'N/runtime', 'N/render', 'N/email', 'N/https'], function (log, record, search, runtime, render, email, https) {
    ...
    function scoreFinal(dni) {
        ...
        try {
            ...
            // EJEMPLO 1
            // currentt2
            var data = JSON.parse(objResponse.body).datosBcu;
            // T6
            ...
            // log.debug("---------", '******************************');
            // log.debug("data T2 - MnPesos", t2_mnPesos);
            // log.debug("data T2 - MePesos", t2_mePesos);
            // log.debug("data T6 - MnPesos", t6_mnPesos);
            // log.debug("data T6 - MePesos", t6_mePesos);

            var endeudamiento = -314;
            try {

                endeudamiento = ((t2_mnPesos + t2_mePesos) / (t6_mnPesos + t6_mePesos)) - 1;
                // log.debug("endeudamiento", endeudamiento);
            ...
            for (var i = 0; i < data.data.EntidadesRubrosValores.length; i++) {
                ...
                if (data.data.EntidadesRubrosValores[i].Calificacion == "2B" || data.data.EntidadesRubrosValores[i].Calificacion == "2" || data.data.EntidadesRubrosValores[i].Calificacion == "3" || data.data.EntidadesRubrosValores[i].Calificacion == "4" || data.data.EntidadesRubrosValores[i].Calificacion == "5") {
                    mensaje = {}//negativo
                    mensaje.logTxt = logTxt;
                    mensaje.error_reglas = true;
            ...
            for (var key in t2) {
                ...
                if (currentt2.Cont && currentt2.NombreEntidad.indexOf("Vizcaya") > -1) {
                    ...
                    // log.debug("currentt2.NombreEntidad.Cont",currentt2.NombreEntidad.Cont)
                    // log.debug("currentt2.NombreEntidad.Cont",!currentt2.NombreEntidad.Cont)
                    // log.debug("tes",currentt2.NombreEntidad.indexOf("Vizcaya") > -1)
            ...
            // log.debug('ent_t6_binned_res', ent_t6_binned_res);
            // log.debug('t6_binned_res', t6_binned_res);
            // log.debug('t6_creditel_binned_res', t6_creditel_binned_res);
            // log.debug('t6_oca_binned_res', t6_oca_binned_res);
            // log.debug('t0_fnb_binned_res', t0_fnb_binned_res);
            // log.debug('t0_asi_binned_res', t0_asi_binned_res);
            // log.debug('t0_bbva_binned_res', t0_bbva_binned_res);
            // log.debug('t6_cred_dir_comp_binned_res', t6_cred_dir_comp_binned_res);
            // log.debug('t6_banco_binned_res', t6_banco_binned_res);
            // log.debug('vig_noauto_t6_coop_binned_res', vig_noauto_t6_coop_binned_res);
            // log.debug('t0_santa_binned_res', t0_santa_binned_res);
            // log.debug('emp_valor_binned_res', emp_valor_binned_res);
            // log.debug('cont_t0_fucac_binned_res', cont_t0_fucac_binned_res);
            // log.debug('brou_grupo_binned_res', brou_grupo_binned_res);
            // log.debug('t6_pronto_binned_res', t6_pronto_binned_res);
            // log.debug('t0_scotia_binned_res', t0_scotia_binned_res);
            // log.debug('cred_dir_binned_res', cred_dir_binned_res);
            ...
            // log.debug('logTxt', '------------------------------------------');
            // log.debug('objetos.logTxt', objetos.logTxt);
