...
define(['N/record', 'N/https', 'N/log', 'N/runtime', 'N/error', 'N/cache', 'N/search'], 
    ...
    /**
     * @author Gerardo Gonzalez
     * @desc getAccessToken - Obtiene token OAuth2 de Equifax (con cache)
    ...
    function getAccessToken(config) {
        ...
        try {
            ...
            if (cachedToken) {
                log.debug('Token obtenido de cache');
                return cachedToken;
            }
        } catch (cacheError) {
            log.debug('Cache no disponible, obteniendo nuevo token', cacheError.message);
        }
        ...
        if (!config.clientId || !config.clientSecret) {
            throw error.create({
                ...
                message: 'Faltan credenciales de Equifax (clientId/clientSecret)',
                notifyOff: true
            });
        }

        // Obtener nuevo token
        log.audit('Solicitando nuevo token OAuth', { url: config.tokenUrl });

        const authHeader = 'Basic ' + encodeBase64(config.clientId + ':' + config.clientSecret);
        ...
        log.debug('Token Response', { code: response.code });

        if (response.code !== 200) {
            throw error.create({
                name: 'TOKEN_ERROR',
                message: 'Error obteniendo token OAuth: HTTP ' + response.code + ' - ' + response.body,
                notifyOff: true
            });
        }
        ...
        try {
            ...
            throw error.create({
                name: 'TOKEN_PARSE_ERROR',
                message: 'Error parseando respuesta de token: ' + e.message,
                notifyOff: true
            });
        }
        ...
        if (!accessToken) {
            throw error.create({
                name: 'TOKEN_MISSING',
                message: 'Token no encontrado en respuesta',
                notifyOff: true
            });
        }
        ...
        try {
            ...
            log.debug('Token guardado en cache');
        } catch (cacheError) {
            log.debug('Error guardando token en cache', cacheError.message);
        }
